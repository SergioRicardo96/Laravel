...
class susccriptores extends Controller
{
    ...
    {//$servi = Servicio::all();
    	...
    	$consulta1 = DB::select('SELECT servicios.nombre,servicios.costo,pagos.* FROM servicios,suscripcions,pagos WHERE servicios.id IN(SELECT suscripcions.servicio_id FROM servicios WHERE suscripcions.id IN(SELECT pagos.suscripcion_id FROM pagos))'); 
        ...
        return view('cobrador\suscriptores',compact("consulta","consulta1","consulta2"));
    }
}
